...
`dsh-dingtalk-status` 通过 `sidebar.footer.action` 注册“钉钉”，复用稳定 foot-area 标记将它放到设置同层的底部右侧；余额独占第一行，设置入口在左侧、钉钉入口在右侧第二行。展开态显示图标与名称及状态点，折叠态保留 36px rail；弹窗按实时侧边栏宽度居中于会话区域。官方 `@dingtalk-real-ai/dsh-dingtalk@0.6.2` 通过 Stream 长连接接收管理员任务，支持续聊、审批、图片，以及 AI Card 流式或 Markdown 最终结果，不需要公网入站端口。默认 workspace 为 `/Users/marcus/Desktop`，发送者仅管理员、群聊关闭、工具关闭、图片模式 `auto`、debug 关闭；缺少凭据时官方插件跳过连接并保持 Host 正常启动。当前官方连接器不支持入站文件、音频、视频，产物仍需在大神本地会话中打开。状态扩展只读官方插件版本、凭据是否存在、管理员绑定、Stream 新鲜度、AI Card 已知能力与绑定会话数量，不返回密钥、staffId、conversationId、sessionWebhook、绑定口令或日志。
