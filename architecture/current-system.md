...
`architecture/dingtalk-control-card.v1.json` 定义“钉钉大神控制台”的五类能力：首页状态、会话、模型与推理、任务控制、产物。已在“大神”应用正式发布独立模板 `Dashen Control Center v1`（`f42ee85c-bdb9-4197-b8b7-4379e9b64878.schema`）；它使用官方动态表单的 `title`、`form_fields`、`form_status`、`button_text`、`err_msg` 合同，与审批/Plan Review 模板隔离。`/menu` 优先投放互动卡片，选择只更新同一卡片，提交才执行会话、模型与推理等级、停止或新会话动作；模型与推理等级作为一组校验，产物和任务各有独立视图。会话目录先按工作区和时间截取 10 条再读取标题，选中操作复用版本化内存槽位；成功后主按钮显示“✓ 设置已应用”，再次修改恢复默认文案。卡片不显示完整 session id、conversation id、staff id、路径、凭据或调试数据；执行前重新从 bindings、`sessionQuery`、`llm` 和 agents 真源解析并校验管理员、版本、幂等与有效期，不建立第二套状态。Android 已完成 `/menu`、会话、模型/推理、产物空状态和任务无动作提交真实验收，回调约 0—400ms、一次点击生效；iOS 按用户决定不纳入本轮验收。模板缺失或卡片失败时降级为 `/menu`、`/status`、`/sessions`、`/session use`、`/models`、`/model use`、`/effort`、`/artifacts`、`/new`、`/stop`、`/help` 命令。底部布局保持余额第一行整行，设置左、钉钉右第二行；折叠态保留 36px rail。
