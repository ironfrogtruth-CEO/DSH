...
`architecture/dingtalk-control-card.v1.json` 定义“钉钉大神控制台”的五类能力：首页状态、会话、模型与推理、任务控制、产物。已在“大神”应用正式发布独立模板 `Dashen Control Center v1`（`f42ee85c-bdb9-4197-b8b7-4379e9b64878.schema`）；它使用官方动态表单的 `title`、`form_fields`、`form_status`、`button_text`、`err_msg` 合同，与审批/Plan Review 模板隔离。`/menu` 优先投放互动卡片，选择只更新同一卡片，提交才执行会话、模型与推理等级、停止或新会话动作；模型与推理等级作为一组校验，产物和任务各有独立视图。任务视图从 `agentPresets` 真源列出工作模式，钉钉默认 `reliable-development`（显示 CyberMarcus），不继承大神 App 当前 Avengers；选择“新会话”只保存 `presetOverrides` 并解除旧绑定，下一条普通消息由 Bridge 创建会话并把所选 ID 写入 header `agentPreset`。会话目录先按工作区和时间截取 10 条再读取标题，选中操作复用版本化内存槽位；成功后主按钮显示对应结果，再次修改恢复默认文案。卡片不显示完整 session id、conversation id、staff id、路径、凭据或调试数据；执行前重新从 bindings、`sessionQuery`、`llm`、`agentPresets` 和 agents 真源解析并校验管理员、版本、幂等与有效期，不建立第二套会话状态。Android 已完成 `/menu`、会话、模型/推理、产物、任务和 CyberMarcus 新会话真实验收，新会话头部回读为 `agentPreset=reliable-development`；iOS 按用户决定不纳入本轮验收。模板缺失或卡片失败时降级为 `/menu`、`/status`、`/sessions`、`/session use`、`/models`、`/model use`、`/effort`、`/artifacts`、`/new`、`/stop`、`/help` 命令。底部布局保持余额第一行整行，设置左、钉钉右第二行；折叠态保留 36px rail。
...
文本任务默认使用 `settings.yaml` 配置的云端智谱模型，也可使用 DeepSeek 云端模型；界面不再提供本地对话模型选择器。canonical 本地模型根仍固定为 `/Users/marcus/Desktop/虾缸/MODEL`，只承载媒体、语音和检索辅助能力。本地 Ollama 保留 FLUX (`x/flux2-klein:4b`) 与 EmbeddingGemma (`embeddinggemma:latest`)；两者由后台适配器按能力静默调用，不进入对话模型选择器，也不承担普通对话压缩策略。

所有用户图片都由 Host 自动接管，不要求用户切换当前主模型：durable 附件先交给 `deepseek-v4-flash-vision-exp`，失败后调用智谱免费视觉链；云端视觉均不可用时明确阻断，不再调用本地对话模型。为让图片到达视觉桥，`deepseek-v4-flash` 与 `deepseek-v4-pro` 在 Host 能力目录中声明 image 准入，但原图不会发送给这两个文本主模型；视觉桥只把带 untrusted-data 边界的精简识图结果投影给当前主模型继续回答。原图与用户文字仍一次提交并保留在 durable 会话。TTS/STT 同样不是 LLM 模型：通过固定 `/Users/marcus/.dsh/bin/dsh-local-ai` 的 `tts`/`stt` 子命令，经 bash 或注册工具路由调用；FLUX、生音频/转写、视频/成片工具（`image_generation_tool`、`tts_tool`、`asr_engine`、`video_composer_tool`）继续由后台适配器或 Host 工具调度。顶层 CyberMarcus 仍可通过后台适配器、`execute_flash` 或具名 Marvel 子智能体调度完整辅助能力。
