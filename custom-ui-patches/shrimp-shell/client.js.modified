    ...
    function apply(ctx) {
      ...
      function ShrimpLibraryView() {
        ...
        const runItem = async (item) => {
          const existingRun = item && item.ref ? runForShrimp(item, runs) : null
          const existingStatus = String(existingRun && (existingRun.status || existingRun.state) || '').toLowerCase()
          if (ACTIVE_RUN_STATUSES.has(existingStatus)) return
        ...
        if (selected) {
          ...
          if (detail && !isDraft) {
            ...
            const controls = h('div', { style: { display: 'grid', gap: 12, marginTop: 14 } },
              ...
              knowledgeBody,
              h('div', { style: { display: 'flex', gap: 8, flexWrap: 'wrap' } }, h('button', { type: 'button', disabled: busy || Boolean(activeRun) || !(selected.capabilities && selected.capabilities.can_run), style: actionButton(true), onClick: () => runItem(selected) }, activeRun ? '运行中…' : '运行这只虾'), latestRun ? h('button', { type: 'button', style: actionButton(false), onClick: () => window.dispatchEvent(new CustomEvent('shrimp:open-artifact', { detail: { runId: latestRun.id, shrimpRef: selected.ref, shrimpDomain: selected.domain } })) }, '打开最近一次产物') : null),
        ...
        const rows = ordered.map((item) => {
          ...
          const rowRun = runForShrimp(item, runs)
          const rowStatus = String(rowRun && (rowRun.status || rowRun.state) || item.state || '').toLowerCase()
          const active = ACTIVE_RUN_STATUSES.has(rowStatus)
      ...
      style.textContent = [
        ...
        '.shrimp-harness-brand { --shrimp-wordmark-y: 6px; --shrimp-delivery-y: -4px; box-sizing: border-box; display: flex !important; align-items: center; justify-content: flex-start; gap: 0; flex: 0 0 224px; min-width: 224px; width: 224px !important; max-width: 224px; height: 70px; padding: 0 !important; overflow: visible !important; }',
        ...
        'html[data-shrimp-desktop="true"] .shrimp-harness-brand { flex: 0 0 216px; min-width: 216px; width: 216px !important; max-width: 216px; height: 66px; gap: 0; margin: 0 auto 0 0; transform: translateY(8px); }',
        ...
        "html[data-shrimp-desktop=\"true\"] .shrimp-harness-brand::after { flex: 0 0 80px; min-width: 80px; height: 28px; padding: 0 10px; border-radius: 6px; font-size: 9.5px; line-height: 1; letter-spacing: .14em; transform: translate(-2px, var(--shrimp-delivery-y)); }",
        ...
        // The two theme assets have different optical (ink-weighted) centers,
        // so DELIVERY uses a theme-specific Y token while the wordmark stays
        // anchored under the macOS traffic lights.
        ".shrimp-harness-brand::before { transform: translateY(var(--shrimp-wordmark-y)); }",
        ".shrimp-harness-brand::after { content: 'DELIVERY'; display: inline-flex; align-items: center; justify-content: center; flex: 0 0 80px; align-self: center; box-sizing: border-box; min-width: 80px; height: 28px; padding: 0 10px; border-radius: 6px; background: var(--dsw-alias-label-primary, #0f1115); color: var(--dsw-alias-label-primary-inverted, #fff); font: 650 9.5px/1 ui-sans-serif, -apple-system, BlinkMacSystemFont, 'SF Pro Text', 'Segoe UI', sans-serif; letter-spacing: .14em; white-space: nowrap; transform: translate(-2px, var(--shrimp-delivery-y)); }",
        "body[data-ds-dark-theme] .shrimp-harness-brand { --shrimp-delivery-y: 2.5px; }",
        ...
        // 图片入口：入口已下线（原图仍可粘贴/拖拽）；识图只在 Host 请求边界后台运行。
        '.shrimp-image-picker { display: none !important; }',
        '.shrimp-voice-btn { display: inline-flex; align-items: center; justify-content: center; width: 30px; height: 30px; margin-left: 2px; border: none; border-radius: 8px; background: transparent; color: var(--dsw-alias-label-secondary, #626870); cursor: pointer; transition: background .15s ease, color .15s ease; } .shrimp-voice-btn:hover { background: var(--dsw-alias-interactive-bg-hover, rgba(128,128,128,.12)); color: var(--dsw-alias-label-primary, #1f2329); } .shrimp-voice-btn svg { width: 16px; height: 16px; flex: 0 0 16px; } .shrimp-voice-btn[data-state="recording"] { color: #e5484d; background: color-mix(in srgb, #e5484d 10%, transparent); } .shrimp-voice-btn[data-state="browser"] { opacity: .6; }',
      ...
      // ---- 语音输入按钮（原生桥；图标化，置于输入框右侧） ----
      ctx.effect(() => slots.inject('conversation.input.right', () => slots.register(
        { name: 'conversation.input.right', id: 'shrimp-voice', order: 1, label: '语音输入' },
        () => {
          ...
          const recording = vstate === 'recording'
          const icon = React.createElement(
            ...
            React.createElement('rect', { x: '9', y: '2.75', width: '6', height: '10.5', rx: '3', fill: recording ? 'currentColor' : 'none' }),
            React.createElement('path', { d: 'M5.5 11v1a6.5 6.5 0 0 0 13 0v-1' }),
            React.createElement('line', { x1: '12', y1: '18.5', x2: '12', y2: '21.25' }),
          )
          return React.createElement(
            'button',
            {
              ...
              'data-state': vstate,
              'aria-label': recording ? '结束语音输入' : '开始语音输入（本地识别）',
              title: native
                ? (recording ? '正在录音，点击结束并填入文字' : '点击开始语音输入（本地识别）')
      ...
      ctx.effect(() => slots.inject('conversation.input.left', () => slots.register(
        ...
        ({ session, input }) => {
          if (true) return null // 图片选择入口已按用户要求下线；原图仍可经粘贴/拖拽进入会话
