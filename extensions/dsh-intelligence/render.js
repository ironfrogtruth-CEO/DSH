...
const OMIT_KEY = /^(?:rawOutput|history|messages|transcript|fullHistory|events|eventLog|toolResults|rawHistory)$/i
...
function sanitize(value, key, seen) {
  ...
  if (value === null || typeof value === 'string' || typeof value === 'boolean') return typeof value === 'string' ? redactText(value) : value
...
export function boundedJson(value, maxChars = DEFAULT_MAX_CHARS) {
  ...
  const previewChars = Math.max(100, maxChars - 120)
  return JSON.stringify({ truncated: true, originalChars: serialized.length, preview: serialized.slice(0, previewChars) })
}

export function renderJson(title, value, maxChars = DEFAULT_MAX_CHARS) {
  return [{ type: 'text', text: `${title}\n${boundedJson(value, maxChars)}` }]
}

export function renderError(value, maxChars = DEFAULT_MAX_CHARS) {
  const code = value?.code || 'INTELLIGENCE_ERROR'
  const message = String(value?.error || value?.message || 'unknown error').slice(0, maxChars - 80)
  return [{ type: 'text', text: `失败: ${code} ${redactText(message)}` }]
}
