...
(function(){
  ...
  function getAssetRoot(){
    return document.body.dataset.assetRoot || window.PA_ASSET_ROOT || 'resources/visual-component-library';
  }
  ...
  function insertFreeText(){
    const slide = currentSlide || deck.querySelector('.slide');
    if(!slide) return;
  ...
  function findAssetTarget(){
    ...
    if(selected.matches('img[data-replacement-group]')) return selected;
    return selected.querySelector('img[data-replacement-group]');
  }

  function updateAssetButtonState(){
    const btn = document.getElementById('replaceAsset');
    if(!btn) return;
    btn.disabled = !findAssetTarget();
  }
  ...
  window.EditableReportDeck = {
    ...
    set selected(v){ selected = v; updateAssetButtonState(); }
  };
